# This document focuses on the implementation of the k-nearest-neighbors (KNN) model for the Heart Disease Dataset and the related formal methods/properties
...
#------------------------------------------------------------------------------------------------------------------------------------------------------
# Import SKLearn Library for splitting the dataset, training a KNN, and calculating the accuracy
...
# Create KNN model and train it on the training subsets (using the fit() function)
knn_model = KNeighborsClassifier(n_neighbors=5)
knn_model.fit(train_features,train_hdv)
...
print(f"The K-Nearest-Neighbors Model has successfully been trained on the Heart Disease Dataset.")
...
# This function prints the KNN model's distance metrics, their sample number and the distance to the training sample for verifying correctness
def print_knn_values(knn_model,knn_features,test_features):
    # Store the distance values and indices for each neighbor
    temp_dist, temp_ind = knn_model.kneighbors(test_features)
    # Print Distance Metrics String
    print("Distance Metrics:")
    # Traverse each sample
    for idx, (distance, index) in enumerate(zip(temp_dist,temp_ind)):
        # Print sample index
        print(f"Sample index {idx}:")
        # Traverse each indice and distance value
        for x, y in zip(distance, index):
            # Print distance
            print(f" Distance to sample {y}: {x:.2f}")

# This function verifies the KNN model using Z3
def verify_knn_correctness(knn_model,knn_features,test_hdv):
    ...
    # Traverse each sample
    for idx, (distance, index) in enumerate(zip(temp_dist,temp_ind)):
        temp_neighbor = train_hdv[index]
        temp_majority = numpy.sum(temp_neighbor) > len(temp_neighbor) / 2
        # Add prediction to Z3
        # z3_solver.add(temp_majority == BoolVal(test_hdv[idx] == 1))
        # Check if prediction is satisfied and increment prediction counter
        # if z3_solver.check() == sat:
            # prediction_counter += 1
        if temp_majority == (test_hdv[idx] == 1):
            prediction_counter += 1
...
def menu_interface():
    while True:
        ...
        print("0. Exit")
        print("1. Correctness: Print KNN model Metrics for Verification")
        ...
        match user_selection:
            case '1':
                print_knn_values(knn_model,train_features,test_features)
            case '2':
                temp = verify_knn_correctness(knn_model,test_features,test_hdv)
                print(f"KNN Model Accuracy after Verification: {temp:.2f}")
            case '3':
                safety_compliance = verify_model_safety(knn_model,test_features,test_hdv)
                print(f"Safety Compliance: {safety_compliance:.2f}")
            case '4':
                robustness_accuracy = verify_model_robustness(knn_model,test_features,test_hdv)
            ...
            case '5':
                ...
                disp_imp_rat, equ_opp_dif = verify_model_fairness(knn_model,test_features,test_hdv,demographic_feature)
                print(f"Disparate Impact Ratio for Model: {disp_imp_rat:.2f}")
                print(f"Equal Opportunity Difference for Model: {equ_opp_dif:.2f}")
            case '6':
                temporal_consistency_acc = verify_model_temporal_consistency(knn_model,features,hdv)
            ...
            case '7':
                generalizability_acc = verify_model_generalizability(knn_model,features,hdv)
                print(f"Model Accuracy after Generalizability Verification: {generalizability_acc:.2f}")
            case '8':
                time_half,time_full,mem_half,mem_full = verify_model_scalability(knn_model,features,hdv)
