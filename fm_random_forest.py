# This document focuses on the implementation of the random forest model for the Heart Disease Dataset and the related formal methods/properties
...
#------------------------------------------------------------------------------------------------------------------------------------------------------
# Import SKLearn Library for splitting the dataset, training a random forest classifier, and calculating the accuracy
...
# Import the tree function for outputting the individual decision tree in the random forest (Correctness)
...
# Create random forest model and train it on the training subsets (using the fit() function)
ranfor_model = RandomForestClassifier(random_state=42)
ranfor_model.fit(train_features,train_hdv)
...
predictions = ranfor_model.predict(test_features)
...
print(f"The Random Forest Model has successfully been trained on the Heart Disease Dataset.")
...
# This function outputs the rules used by the decision trees in the random forest
def print_randforest_rules(randforest,randforest_features):
    # Iterate over tree
    for i, tree in enumerate(randforest.estimators_):
        # Print tree rules string
        print(f"Tree {i+1} rules:")
        # Call function for printing decision tree rules
        print_dectree_rules(tree,randforest_features)
        # Print newline for readability
        print("\n")

def verify_randforest_correctness(randforest,test_features,test_hdv):
    ...
    for idx, row in test_features.iterrows():
        ...
        if idx >= len(test_hdv):
            ...
            break
        # Store predictions from each tree in list
        pred_store = []
        ...
        for tree in randforest.estimators_:
            ...
            temp_prediction = traverse_tree(0,1,row)
            # Add the expression to Z3 solver
            z3_solver.add(temp_prediction == BoolVal(test_hdv[idx] == 1))
            # Check if the logical expression is satisfiable and append correct boolean to prediction list
            if z3_solver.check() == sat:
                pred_store.append(True)
            else:
                pred_store.append(False)
            # Define a variable to compute the majority vote of predictions
            pred_vote = sum(pred_store) > len(pred_store)/2
            if pred_vote == (test_hdv[idx] == 1):
...
def menu_interface():
    while True:
        ...
        print("0. Exit")
        print("1. Correctness: Print Random Forest Decision Tree Rules")
        ...
        match user_selection:
            case '1':
                print("Random Forest Rules:")
                print_randforest_rules(ranfor_model,features.columns)
            case '2':
                temp = verify_randforest_correctness(ranfor_model,test_features,test_hdv)
                print(f"Random Forest Accuracy after Verification: {temp:.2f}")
            case '3':
                safety_compliance = verify_model_safety(ranfor_model,test_features,test_hdv)
                print(f"Safety Compliance: {safety_compliance:.2f}")
            case '4':
                robustness_accuracy = verify_model_robustness(ranfor_model,test_features,test_hdv)
            ...
            case '5':
                ...
                disp_imp_rat, equ_opp_dif = verify_model_fairness(ranfor_model,test_features,test_hdv,demographic_feature)
                print(f"Disparate Impact Ratio for Model: {disp_imp_rat:.2f}")
                print(f"Equal Opportunity Difference for Model: {equ_opp_dif:.2f}")
            case '6':
                temporal_consistency_acc = verify_model_temporal_consistency(ranfor_model,features,hdv)
            ...
            case '7':
                generalizability_acc = verify_model_generalizability(ranfor_model,features,hdv)
                print(f"Model Accuracy after Generalizability Verification: {generalizability_acc:.2f}")
            case '8':
                time_half,time_full,mem_half,mem_full = verify_model_scalability(ranfor_model,features,hdv)
