# This document focuses on the implementation of the support vector machine model (SVM) for the Heart Disease Dataset and the related formal methods/properties
...
#------------------------------------------------------------------------------------------------------------------------------------------------------
# Import SKLearn Library for splitting the dataset, training a SVM, and calculating the accuracy
...
# Create SVM tree model and train it on the training subsets (using the fit() function)
svm_model = SVC(kernel='linear',random_state=42)
svm_model.fit(train_features,train_hdv)
...
print(f"The Support Vector Machine Model has successfully been trained on the Heart Disease Dataset.")
...
# This function prints the SVM model's separation margins for each coefficient and the intercept for correctness verification
def print_svm_values(svm_model,svm_features):
    # Get SVM intercept and model coefficients
    temp_intercept = svm_model.intercept_[0]
    temp_coefs = svm_model.coef_[0]
    # Traverse each feature with its relevant coefficient
    for feature, coef in zip(svm_features,temp_coefs):
        # Print feature and coefficient
        print(f"{feature}: {coef:.4f}")
    # Print intercept
    print(f"Intercept: {temp_intercept:.4f}")
# This function verifies the SVM model using Z3
def verify_svm_correctness(svm_model,svm_features,test_hdv):
    ...
    for idx, row in svm_features.iterrows():
        # Compute and store linear combination of model
        temp_comb = sum(temp_coefs*row[feature] for temp_coefs, feature in zip(temp_coefs,svm_features.columns))+temp_intercept
...
def menu_interface():
    while True:
        ...
        print("0. Exit")
        print("1. Correctness: Print SVM Separation Margins for Verification")
        ...
        match user_selection:
            case '1':
                print("Separation Marings of SVM Model:")
                print_svm_values(svm_model,features.columns)
            case '2':
                temp = verify_svm_correctness(svm_model,test_features,test_hdv)
                print(f"SVM Accuracy after Verification: {temp:.2f}")
            case '3':
                safety_compliance = verify_model_safety(svm_model,test_features,test_hdv)
                print(f"Safety Compliance: {safety_compliance:.2f}")
            case '4':
                robustness_accuracy = verify_model_robustness(svm_model,test_features,test_hdv)
            ...
            case '5':
                ...
                disp_imp_rat, equ_opp_dif = verify_model_fairness(svm_model,test_features,test_hdv,demographic_feature)
                print(f"Disparate Impact Ratio for Model: {disp_imp_rat:.2f}")
                print(f"Equal Opportunity Difference for Model: {equ_opp_dif:.2f}")
            case '6':
                temporal_consistency_acc = verify_model_temporal_consistency(svm_model,features,hdv)
            ...
            case '7':
                generalizability_acc = verify_model_generalizability(svm_model,features,hdv)
                print(f"Model Accuracy after Generalizability Verification: {generalizability_acc:.2f}")
            case '8':
                time_half,time_full,mem_half,mem_full = verify_model_scalability(svm_model,features,hdv)
